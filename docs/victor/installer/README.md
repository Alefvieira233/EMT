...
- O fluxo de desenvolvimento continua separado. O seu `.addin` local do repositorio não é alterado.
- O pacote usa a compilacao `Release` por padrão.
...
- O `setup.exe` é um bootstrapper proprio, compilado em .NET e empacotado de forma separada do add-in principal.
