...
Todos ficam em `%LOCALAPPDATA%\SteelBIM\` (geralmente
`C:\Users\<seu-usuario>\AppData\Local\SteelBIM`):
...
1. Abra o Revit como administrador uma única vez — o add-in precisa criar
   `%LOCALAPPDATA%\SteelBIM\` na primeira execução.
2. Confirme que o arquivo `.addin` está em
   `%APPDATA%\Autodesk\Revit\Addins\<versão>\SteelBIM.addin` e que o caminho
   apontado lá existe.
3. Verifique o log em `%LOCALAPPDATA%\SteelBIM\logs\` — procure por linhas
   com `[ERR]` ou `[FATAL]` no arquivo do dia.

### "Licença expirada" ou "Licença inválida"

1. Abra o comando `Sobre SteelBIM` na ribbon — ele mostra status, validade
   e user ID da licença carregada.
2. Se a mensagem for *"Arquivo de licença não encontrado"*, cole o arquivo
   `license.key` recebido em `%LOCALAPPDATA%\SteelBIM\`.
...
4. Se você é desenvolvedor e quer usar um segredo próprio para testes, crie
   `%LOCALAPPDATA%\SteelBIM\license.secret` com a string do segredo
...
2. Se travou de verdade, force o encerramento. Na próxima abertura, verifique
   `%LOCALAPPDATA%\SteelBIM\crashes\` — se há um arquivo novo, anexe-o
   ao bug report.
...
2. Versão do plugin (comando *Sobre SteelBIM* na ribbon).
3. Log do dia em `%LOCALAPPDATA%\SteelBIM\logs\` (compacte em zip).
4. Se houve crash, o arquivo mais recente em `%LOCALAPPDATA%\SteelBIM\crashes\`.
...
```powershell
Remove-Item "$env:LOCALAPPDATA\SteelBIM\logs\ferramenta-emt-*.log" -Force
```
...
```powershell
Remove-Item "$env:LOCALAPPDATA\SteelBIM\crashes\*.txt" -Force
```
...
*Não apague `%LOCALAPPDATA%\SteelBIM\`* — sua licença mora lá.
...
- Logs são escritos via Serilog wrapper (`SteelBIM/Infrastructure/Logger.cs`).
...
- `backup-victor-pre-merge.zip` (302 MB) — snapshot do fork do Victor antes do
  ...
  `C:\Users\User\Downloads\FerramentaEMT-archive\` (paths literais do PC
  antes do rebrand v2.0.0; preservados como historico).
