...
{
    internal static class TopRebarService
    {
        public static int Generate(
            Document doc, Element host, BlocoHostFrame frame,
            BlocoBarraSuperiorConfig config)
        {
            int created = 0;

            if (config.BarsX.Ativo && !string.IsNullOrWhiteSpace(config.BarsX.BarTypeName))
                created += GenerateDirection(doc, host, frame, config.BarsX, isAlongX: true, "Superior-X");

            if (config.BarsY.Ativo && !string.IsNullOrWhiteSpace(config.BarsY.BarTypeName))
                created += GenerateDirection(doc, host, frame, config.BarsY, isAlongX: false, "Superior-Y");
            ...
            BlocoBarraDirecaoConfig cfg, bool isAlongX, string label)
        {
            ...
            double z = frame.MaxZ - cov - ToCm(cfg.OffsetZCm);
